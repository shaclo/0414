...
3. 左侧控制面板可调整：
   - **爽感公式**：勾选具体的爽感模式（小爽/中爽/大爽）
   - **钩子公式**：勾选结尾悬念类型（动作截断/身份炸弹/危机降临等）
   - **爽感节奏参数**：设定小爽/中爽/大爽的集数间隔，系统根据集号自动调度
   - **时长区间**：设定每集时长范围，自动换算目标字数
4. 可在编辑框内进一步修改，满意后锁定
...
│   └── updater.py                   # GitHub Release 自动更新（检查/下载/解压/重启）
│
├── ui/                              # 界面层
